...
export default function BlogPage() {
  ...
  return <main className="front-blog-page">
    ...
    <section className="front-filter-panel" aria-label="文章筛选">
      <div className="front-filter-row"><strong>主题</strong><div className="front-filter-options"><button type="button" className={!selectedType ? 'active' : ''} onClick={clearFilters}>全部主题</button>{groups.map((group) => <button type="button" className={selectedType === group.type.id ? 'active' : ''} key={group.type.id} onClick={() => resetAnd(() => { setSelectedType(group.type.id); setSelectedCategory(0); setQuery(''); setSearchInput('') })}>{group.type.name}<small>{group.categories.length}</small></button>)}</div></div>
      {selectedType > 0 && <div className="front-filter-row front-secondary-filter"><strong>细分</strong><div className="front-filter-options"><button type="button" className={!selectedCategory ? 'active' : ''} onClick={() => resetAnd(() => setSelectedCategory(0))}>全部细分</button>{activeChildren.map((category) => <button type="button" className={selectedCategory === category.id ? 'active' : ''} key={category.id} onClick={() => resetAnd(() => setSelectedCategory(category.id))}>{category.name}</button>)}</div></div>}
    </section>
